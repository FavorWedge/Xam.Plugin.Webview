...
namespace Xam.Plugin.WebView.Abstractions
{
    public partial class FormsWebView : View, IFormsWebView, IDisposable
    {
        ...
        /// <summary>
        /// Fired when a link is clicked. If it desired to open the link in a seperate
        /// browser app, then set NavigationResponseArgs.OffloadOntoDevice to true 
        /// </summary>
        public event EventHandler<NavigationResponseArgs> OnUrlLinkClicked;

        /// <summary>
        /// Fired when navigation begins, for example when the source is set.
        /// </summary>
        public event EventHandler<NavigationResponseArgs> OnNavigationStarted;
        ...
        /// </summary>
        public event EventHandler<NavigationResponseArgs> OnNavigationCompleted;

        /// <summary>
        /// Fires when navigation fires an error. By default this uses the native systems error codes.
        /// </summary>
        public event EventHandler<NavigationResponseArgs> OnNavigationError;
        ...
        internal NavigationResponseArgs HandleUrlLinkClicked(string uri)
        {
            var response = new NavigationResponseArgs { Uri = uri };
            OnUrlLinkClicked?.Invoke(this, response);
            return response;
        }

        internal NavigationResponseArgs HandleNavigationStartRequest(string uri)
        {
            ...
            var response = new NavigationResponseArgs
            {
                Uri = uri,
                OffloadOntoDevice = validUri && !validScheme
            };

            OnNavigationStarted?.Invoke(this, response);
            return response;
        }
        ...
        internal void HandleNavigationCompleted(string uri)
        {
            ...
            OnNavigationCompleted?.Invoke(this, response);
        }

        /// <summary>
        /// NavigationErrors are treated differently on different platforms. If there are errors
        /// when loading the SourceUrl page all platforms return an error. But Android will
        /// report all errors, even after a page has loaded (including errors within javascript accesses
        /// that might return 404 after the page has loaded). For errors occuring when loading SourceUrl
        /// sourceUrlError will be true;
        /// </summary>
        /// <param name="errorCode"></param>
        /// <param name="sourceUrlError">Error occured loading SourceUrl</param>
        internal void HandleNavigationError(int errorCode, bool sourceUrlError)
        {
            var response = new NavigationResponseArgs
            {
                NavigationResult = errorCode,
                Cancel = sourceUrlError,
            };
            OnNavigationError?.Invoke(this, response);
        }
