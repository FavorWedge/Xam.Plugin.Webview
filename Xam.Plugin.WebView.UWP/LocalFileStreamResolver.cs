...
namespace Xam.Plugin.WebView.UWP
{
    public class LocalFileStreamResolver : IUriToStreamResolver
    {
        ...
        {
            if (!_reference.TryGetTarget(out FormsWebViewRenderer renderer))
            ...
            try
            {
                ...
                var f = await StorageFile.GetFileFromApplicationUriAsync(
                    new Uri(string.Concat(renderer.GetBaseUrl(), path)));
