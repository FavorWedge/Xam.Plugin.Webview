...
namespace Xam.Plugin.WebView.iOS
{
    ...
    {
        readonly WeakReference<FormsWebViewRenderer> _reference;
        ...
        {
            if (_reference == null || !_reference.TryGetTarget(out FormsWebViewRenderer renderer)) return;
            if (renderer.Element == null) return;

            var response = new NavigationResponseArgs { Uri = navigationAction.Request.Url.ToString() };
            if (navigationAction.NavigationType == WKNavigationType.LinkActivated &&
                !string.IsNullOrEmpty(navigationAction.Request.Url.Host))
                response = renderer.Element.HandleUrlLinkClicked(response.Uri);

            if( !response.OffloadOntoDevice)
                response = renderer.Element.HandleNavigationStartRequest(navigationAction.Request.Url.ToString());
            ...
            Action<WKNavigationResponsePolicy> decisionHandler)
        {
            ...
            if (navigationResponse.Response is NSHttpUrlResponse)
            {
                ...
                if (code >= 400)
                {
                    ...
                    renderer.Element.HandleNavigationError((int) code, false);
        ...
        public override void DidFailProvisionalNavigation(WKWebView webView, WKNavigation navigation, NSError error)
        {
            ...
            if (renderer.Element == null) return;
            renderer.Element.Navigating = false;
            renderer.Element.HandleNavigationError((int)error.Code, true);
        }
    }
}